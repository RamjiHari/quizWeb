...
if($_REQUEST["action"]=='topics'){
	...
	$get_topics=mysqli_query($con,"SELECT * FROM `quizz_topics` where qt_createdby='$createdby'");
...
if($_REQUEST["action"]=='category'){
	...
	if(!empty($id)){
     $sql="UPDATE `quizz_categories` SET `catg_name`='$category' ,`catg_qt_id`='$topic_id' ,`catg_createdby`='$createdby'  WHERE `catg_id`='$id'";
	}else{
	$sql = "INSERT INTO `quizz_categories`(`catg_name`,`catg_qt_id`,`catg_createdby`) VALUES('$category','$topic_id','$createdby')";
}
